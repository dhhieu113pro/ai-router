...
public sealed class OpenAiTransportCoverageEdgeTests
{
    ...
    public async Task Responses_uses_absolute_override_without_authorization_and_rewrites_body()
    {
        ...
        var definition = Definition() with { BaseUrl = "https://upstream.test/v1", ApiKey = null, ResponsesEndpoint = "https://responses.test/custom" };
    ...
    {
        var provider = Create(new RecordingHandler(_ => Json(HttpStatusCode.ServiceUnavailable, "{}")), Definition() with { Models = ["configured"] });
    ...
    {
        var provider = Create(new RecordingHandler(_ => Json(HttpStatusCode.OK, "{\"data\":{}}")), Definition() with { Models = ["configured"] });
        Assert.Equal(["configured"], await provider.ListModelsAsync());
    }
    ...
    {
        var provider = Create(new RecordingHandler(_ => Json(HttpStatusCode.OK, "not-json")), Definition() with { Models = ["configured"] });
        Assert.Equal(["configured"], await provider.ListModelsAsync());
    }
    ...
    {
        var provider = Create(new RecordingHandler((_, _) => throw new HttpRequestException("network")), Definition() with { Models = ["configured"] });
        Assert.Equal(["configured"], await provider.ListModelsAsync());
    }
    ...
    {
        var handler = new RecordingHandler(async (_, ct) => { await Task.Delay(Timeout.InfiniteTimeSpan, ct); return Json(HttpStatusCode.OK, "{}"); });
        var provider = Create(handler);
    ...
    {
        var handler = new RecordingHandler(_ => Json(HttpStatusCode.OK, "{\"data\":[null,{}, {\"id\":1},{\"id\":\"\"},{\"id\":\"M\"},{\"id\":\"m\"}]}"));
        Assert.Equal(["M"], await Create(handler).ListModelsAsync());
    }

    [Fact]
    public async Task Health_reports_non_success_status()
    {
        var health = await Create(new RecordingHandler(_ => Json(HttpStatusCode.BadGateway, "{}"))).CheckHealthAsync();
    ...
    {
        var health = await Create(new RecordingHandler((_, _) => throw new HttpRequestException("offline"))).CheckHealthAsync();
        Assert.False(health.Success);
    ...
    {
        var handler = new RecordingHandler(async (_, ct) => { await Task.Delay(Timeout.InfiniteTimeSpan, ct); return Json(HttpStatusCode.OK, "{}"); });
        var result = await Create(handler, Definition() with { Timeout = TimeSpan.FromMilliseconds(10) }).SendChatAsync("model", Request, false);
    ...
    {
        var result = await Create(new RecordingHandler((_, _) => throw new HttpRequestException("network"))).SendChatAsync("model", Request, false);
        Assert.False(result.Success);
    ...
    public async Task Error_string_and_retry_after_date_are_preserved()
    {
        ...
        var handler = new RecordingHandler(_ => { var response = Json(HttpStatusCode.TooManyRequests, "{\"error\":\"busy\"}"); response.Headers.RetryAfter = new RetryConditionHeaderValue(retry); return response; });
        var result = await Create(handler).SendChatAsync("model", Request, false);
    ...
    public async Task Retry_after_delta_is_converted_to_date()
    {
        ...
        var handler = new RecordingHandler(_ => { var response = Json(HttpStatusCode.TooManyRequests, "{}"); response.Headers.RetryAfter = new RetryConditionHeaderValue(TimeSpan.FromSeconds(30)); return response; });
        var result = await Create(handler).SendChatAsync("model", Request, false);
        Assert.NotNull(result.RetryAfter);
    ...
    {
        var response = new HttpResponseMessage(HttpStatusCode.BadRequest) { Content = new StringContent("raw upstream failure") };
        var result = await Create(new RecordingHandler(_ => response)).SendChatAsync("model", Request, false);
    ...
    {
        var response = new HttpResponseMessage(HttpStatusCode.BadRequest) { ReasonPhrase = "Custom reason", Content = new StringContent(string.Empty) };
        var result = await Create(new RecordingHandler(_ => response)).SendChatAsync("model", Request, false);
        Assert.Equal("Custom reason", result.ErrorMessage);
    }

    [Fact]
    public async Task Unknown_error_status_uses_target_failure_classification()
    {
        var result = await Create(new RecordingHandler(_ => Json((HttpStatusCode)418, "{}"))).SendChatAsync("model", Request, false);
        Assert.Equal(ProviderFailureKind.TargetFailure, result.FailureKind);
    }
    ...
    public async Task Streaming_result_delegates_full_stream_surface_and_disposes_async()
    {
        ...
        var handler = new RecordingHandler(_ => new HttpResponseMessage(HttpStatusCode.OK) { Content = new DirectStreamContent(inner) });
        var result = await Create(handler).SendChatAsync("model", Request, true);
    ...
    public async Task Streaming_result_supports_synchronous_disposal()
    {
        ...
        var result = await Create(new RecordingHandler(_ => new HttpResponseMessage(HttpStatusCode.OK) { Content = new StreamContent(inner) })).SendChatAsync("model", Request, true);
    ...
    {
        var provider = Create(new RecordingHandler(_ => new HttpResponseMessage(HttpStatusCode.OK) { Content = new ThrowingReadStreamContent() }));
        await Assert.ThrowsAsync<IOException>(() => provider.SendChatAsync("model", Request, true));
    }

    private static ProviderDefinition Definition() => new("primary", "Primary", "openai-compatible", "https://upstream.test/v1/", "secret");
    private static OpenAiCompatibleProvider Create(RecordingHandler handler, ProviderDefinition? definition = null) => new(definition ?? Definition(), new StaticHttpClientFactory(new HttpClient(handler)));
    ...
    private sealed class RecordingHandler : HttpMessageHandler
    {
        ...
        public RecordingHandler(Func<HttpRequestMessage, HttpResponseMessage> response) : this((request, _) => Task.FromResult(response(request))) { }
    ...
    private sealed class DirectStreamContent(Stream inner) : HttpContent
    {
        ...
        protected override Task<Stream> CreateContentReadStreamAsync() => Task.FromResult(inner);
        protected override Task<Stream> CreateContentReadStreamAsync(CancellationToken cancellationToken) => Task.FromResult(inner);
    }
    ...
    private sealed class ThrowingReadStreamContent : HttpContent
    {
        ...
        protected override Task<Stream> CreateContentReadStreamAsync() => Task.FromException<Stream>(new IOException("stream open failed"));
        protected override Task<Stream> CreateContentReadStreamAsync(CancellationToken cancellationToken) => Task.FromException<Stream>(new IOException("stream open failed"));
    }
}
